...
export function YouTubeHashtagsGenerator({ onToast }: YouTubeHashtagsGeneratorProps) {
  ...
  const canSubmit = input.trim().length > 0 && !isOverWordLimit && !isGenerating
  ...
  const handleGenerate = async () => {
    ...
    try {
      const result = await generateYouTubeHashtags(input)
    ...
    <div className="content-stack">
      <section className="panel glass-panel">
        <div>
          ...
          <p>Paste your video script, topic, or description below. We will generate optimized YouTube hashtags.</p>
        </div>
      ...
      <section className="panel glass-panel">
        <div className="split-header">
          <div>
            ...
            <p>Review the SEO-optimized hashtags below, then copy them into YouTube Studio.</p>
          </div>
          {generatedHashtags ? (
        ...
        {generatedHashtags ? (
          <article className="result-card hashtags-output">
            <p>{generatedHashtags}</p>
          </article>
        ) : (
